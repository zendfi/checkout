...
export function BankPaymentFlow({ onBack, customerEmail }: BankPaymentFlowProps) {
    ...
    return (
      <div className="animate-slide-down">
        <div className="text-center py-6 sm:py-8">
          ...
          <h4 className="text-xs sm:text-sm font-semibold text-gray-900 mb-1">Sending Verification Code</h4>
          <p className="text-[10px] sm:text-xs text-gray-500">Please wait...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="animate-slide-down">
        <div className="text-center mb-3 sm:mb-4">
          ...
          <h4 className="text-xs sm:text-sm font-semibold text-gray-900 mb-1">Check Your Email</h4>
          <p className="text-[10px] sm:text-xs text-gray-500">
            We sent a verification code to<br />
            <span className="font-medium text-gray-700 text-[10px] sm:text-xs break-all">{customerEmail}</span>
          </p>
        </div>

        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 mb-3 sm:mb-4 flex items-start gap-2">
        ...
        <div className="mb-3 sm:mb-4">
          <label className="text-[10px] sm:text-xs font-medium text-gray-500 mb-2 block text-center">
          ...
          <div className="flex justify-center gap-2 sm:gap-3" onPaste={handleOtpPaste}>
            {otp.map((digit, index) => (
              <input
                ...
                autoFocus={index === 0}
                className={`
                  w-12 h-14 sm:w-11 sm:h-12 text-center text-xl sm:text-lg font-semibold
                  border-2 rounded-xl sm:rounded-lg transition-all duration-150
          ...
          <p className="text-[9px] sm:text-[10px] text-gray-400 text-center mt-2">
    ...
    return (
      <div className="animate-slide-down">
        <div className="text-center mb-3 sm:mb-4">
          ...
          <h4 className="text-xs sm:text-sm font-semibold text-gray-900 mb-1">Transfer to This Account</h4>
          <p className="text-[10px] sm:text-xs text-gray-500">
            Send exactly <span className="font-semibold text-gray-900">₦{bankOrder.fiat_amount.toLocaleString()}</span> to complete
          </p>
        </div>

        {/* Bank Details Card */}
        <div className="bg-gray-50 rounded-xl sm:rounded-lg p-3 sm:p-3 mb-3 space-y-2.5 sm:space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-[10px] sm:text-xs text-gray-500">Bank Name</span>
            <span className="text-[10px] sm:text-xs font-medium text-gray-900">{bankOrder.bank_name}</span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="text-[10px] sm:text-xs text-gray-500">Account Name</span>
            <span className="text-[10px] sm:text-xs font-medium text-gray-900 text-right max-w-[160px] sm:max-w-none truncate">{bankOrder.bank_account_name}</span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="text-[10px] sm:text-xs text-gray-500">Account Number</span>
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="text-sm sm:text-sm font-mono font-bold text-gray-900">
                {bankOrder.bank_account_number}
              </span>
              <button
                onClick={() => handleCopy(bankOrder.bank_account_number, 'account')}
                className="p-1.5 sm:p-1 hover:bg-gray-200 rounded-lg sm:rounded transition-colors active:bg-gray-300"
              >
                {copied === 'account' ? (
          ...
          <div className="border-t border-gray-200 pt-2.5 sm:pt-2 mt-2.5 sm:mt-2">
            <div className="flex justify-between items-center">
              <span className="text-[10px] sm:text-xs text-gray-500">Amount (NGN)</span>
              <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        {/* Waiting indicator */}
        <div className="bg-blue-50 border border-blue-200 rounded-xl sm:rounded-md p-3 mb-3">
          <div className="flex items-center gap-2 mb-1">
            <Loader2 className="w-4 h-4 text-blue-600 animate-spin flex-shrink-0" />
            <span className="text-[10px] sm:text-xs font-medium text-blue-700">Waiting for your transfer...</span>
          </div>
        ...
        {/* Important notes */}
        <div className="text-[10px] sm:text-xs text-gray-500 space-y-1.5 sm:space-y-1 mb-3">
          <p className="flex items-start gap-1.5 sm:gap-1">
            <span className="text-amber-500 flex-shrink-0">•</span>
            <span>Transfer the exact amount shown above</span>
          </p>
          <p className="flex items-start gap-1.5 sm:gap-1">
            <span className="text-amber-500 flex-shrink-0">•</span>
            <span>Use your bank app or USSD to transfer</span>
          </p>
          <p className="flex items-start gap-1.5 sm:gap-1">
            <span className="text-amber-500 flex-shrink-0">•</span>
        ...
        <button
          onClick={onBack}
          className="btn btn-ghost w-full text-[10px] sm:text-xs py-2.5 sm:py-2"
        >
  ...
  if (step === 'completed') {
    return (
      <div className="animate-slide-down">
        <div className="text-center py-6 sm:py-8">
          <div className="w-12 h-12 sm:w-12 sm:h-12 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-2 sm:mb-3">
            <Check className="w-6 h-6 sm:w-6 sm:h-6 text-green-600" />
          </div>
          <h4 className="text-sm sm:text-sm font-semibold text-gray-900 mb-1">Payment Received!</h4>
          <p className="text-xs sm:text-xs text-gray-500">Thank You!</p>
        </div>
      </div>
    );
  }
